...
import { Download, Video, Wand2, Volume2, VolumeX } from 'lucide-react'
import { useModal } from '@/hooks/useModal'

const reels = [
  { id: 1, src: '/videos/reels/Seva-vertical-8-Mb.mp4', label: 'Ожившие фото → нейро' },
          ...
          {reels.map((reel, idx) => (
            <div key={reel.id}>
              ...
              >
                ...
                {/* Кнопка Play/Pause поверх видео (показывается всегда, когда видео на паузе или при наведении?) 
                    Сделаем так: иконка воспроизведения показывается, когда видео на паузе, а при наведении на десктопе появляются контролы */}
                {!playingStates[idx] && (
                ...
                <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition z-20" onClick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    aria-label={mutedStates[idx] ? 'Включить звук' : 'Выключить звук'}
                  >
